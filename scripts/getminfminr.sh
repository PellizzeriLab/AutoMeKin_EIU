...
##ndis forward
   awk '/Pro/{n=0};NF==4{++n;a[n]=$0};END{print '$natom';print "";for(i=1;i<=n;i++) print a[i]}' $tsdirll/IRC/${name}_ircf.xyz > mingeom.xyz
...
##ndis reverse 
   awk '/Pro/{n=0};NF==4{++n;a[n]=$0};END{print '$natom';print "";for(i=1;i<=n;i++) print a[i]}' $tsdirll/IRC/${name}_ircr.xyz > mingeom.xyz
   createMat.py mingeom.xyz 3 $nA
   echo "1" $natom > sprint.dat
   cat ConnMat    >> sprint.dat
   if [ $(sprint2.exe<sprint.dat | awk '/Results for the Laplacian/{getline;n=0;for(i=6;i<=NF;i++) {if($i<='$nfrag_th')++n}};END{print n}') -gt  1 ]; then
